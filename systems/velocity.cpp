...
static ecs::system_base const& vel_input = ecs::make_system([] (input const& input) {
...
static ecs::system_base const& apply_velocity = ecs::make_parallel_system([](particle &par, velocity const& vel, frame_context const& fc) {
    par.x += vel.x * fc.dt;
    par.y += vel.y * fc.dt;
});
